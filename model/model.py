...
class TriAANVC(nn.Module):
    ...
    def __init__(self, encoder_params, decoder_params):
        ...
        self.speechbg_decoder = SpeechBGDecoderDecoder(c_in=256, 
                                                       c_out=encoder_params.c_out,
                                                       num_layer=encoder_params.num_layer,
                                                        c_h=encoder_params.c_h)
    ...
    def forward(self, src, src_lf0, trg):
        ...
        speech_src, bg        = self.speechbg_decoder(src)
        print (speech_src.shape)
        trg, trg_skips = self.spk_encoder(trg)  # target: spk
        print (speech_src.shape, src_emds.shape)
        src    = torch.cat([speech_src.transpose(1,2), src_emds], dim=-1)
        ...
        return output, speech_src, bg
    
class SpeechBGDecoderBlock(nn.Module):
    def __init__(self, c_in, c_out):
        super().__init__()
        self.conv = nn.Sequential(
            nn.ConvTranspose1d(c_in, c_out, kernel_size=3, stride=1, padding=1, output_padding=0),
...
class SpeechBGDecoderDecoder(nn.Module):
    def __init__(self, c_in, c_out, num_layer, c_h):
        super().__init__()
        print ('Expecting basic conv', c_in, c_h)
        c_h=16
        c_in=128
        # self.in_conv = BasicConv(c_in, c_h, kernel_size=3, stride=2, padding=1, relu=True, bn=True)
        self.deconv_blocks = nn.ModuleList()
        in_channels = c_h
        out_channels = c_h // 2
        # Shared deconvolution layers
        for _ in range(4):
            self.deconv_blocks.append(SpeechBGDecoderBlock(in_channels, out_channels))
            in_channels = out_channels
            out_channels = out_channels * 2
            print (out_channels)

        # Speech and background output heads
        self.speech_out_conv = nn.Sequential(
            BasicConv(in_channels, 16, kernel_size=3, stride=1, padding=1, relu=True, bn=True),
            BasicConv(16, 16, kernel_size=3, stride=1, padding=1, relu=True, bn=True),
            # BasicConv(in_cha4nnels, 4, kernel_size=3, stride=2, padding=1, relu=True, bn=True)
        )
        self.background_out_conv = nn.Sequential(
            nn.ConvTranspose1d(in_channels, 256, kernel_size=3, stride=1, padding=1, output_padding=0),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        # Generate speech and background outputs
        speech_out = self.speech_out_conv(x)
        background_out = self.background_out_conv(x)
        return speech_out, background_out
