...
class Trainer:
    ...
    def _run_epoch(self, data_loader, valid=False):
        ...
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            utt_feat = src_feat[:, 256:,:]
            utt_lf0 = src_lf0[:, 128:]
            utt_mel = src_mel[:, 80:,:]

            src_feat = src_feat[:, :256,:]
            src_lf0 = src_lf0[:, :128]
            src_mel = src_mel[:, :80,:]
            print (src_feat.shape, src_lf0.shape, src_mel.shape)
            # exit()

            output, speech, bg = self.model(src_feat, src_lf0, src_feat)
            ...
            if self.cfg.train.siam:
                ...
                output_siam, speech, bg = self.model(siam_x, siam_lf0, siam_x)
                ...
                loss4 = self.criterion(speech, utt_mel)
