...
class TrainDataset(Dataset):
    def __init__(self, cfg, mode):
        ...
        self.utt_path = Path(cfg.utt_path)
        self.utt_n_frames  = cfg.setting.n_frames
        self.utt_speakers  = sorted(os.listdir(self.utt_path/f'{mode}/mels'))
        self.utt_metadata  = Read_json(self.utt_path/f'{mode}.json') # [info (mel_len, speaker, wav_path, mel_path, lf0_path, cpc_path, txt, txt_path, test_type)]
        # mel_stats = np.load(cfg.utt_path + '/mel_stats.npy')

        self.urb_path = Path(cfg.urb_path)
        self.urb_n_frames  = cfg.setting.n_frames
        self.urb_speakers  = sorted(os.listdir(self.urb_path/f'{mode}/mels'))
        self.urb_metadata  = Read_json(self.urb_path/f'{mode}.json') # [info (mel_len, speaker, wav_path, mel_path, lf0_path, cpc_path, txt, txt_path, test_type)]
        # mel_stats = np.load(cfg.urb_path + '/mel_stats.npy')
    ...
    def sample_frame(self, feat, mel, lf0, pos):
    ...
    def __getitem__(self, index):
        ...
        mel_path = self.utt_metadata[index]['mel_path']
        lf0_path = self.utt_metadata[index]['lf0_path']
        if self.cfg.train.cpc:
            feat_path = self.utt_metadata[index]['cpc_path']
            utt_feat      = np.load(feat_path).T
        else:
            feat_path = self.utt_metadata[index]['mel_path']
            utt_feat      = np.load(feat_path).T
            utt_feat      = (utt_feat - self.mean) / (self.std + self.eps)
            
        utt_mel  = np.load(mel_path).T
        utt_mel  = (utt_mel - self.mean) / (self.std + self.eps)
        utt_lf0  = np.load(lf0_path)

        if self.n_frames > feat.shape[-1]:
            feat_len = feat.shape[-1]
            utt_feat     = np.tile(utt_feat, int(np.ceil(self.n_frames / feat_len)))
            utt_mel      = np.tile(utt_mel, int(np.ceil(self.n_frames / feat_len)))
            utt_lf0      = np.tile(utt_lf0, int(np.ceil(self.n_frames / feat_len)))

        utt_lf0            = self.normalize_lf0(utt_lf0)



        
        pos  = random.randint(0, feat.shape[-1] - self.n_frames)
        feat, mel, lf0 = self.sample_frame(feat, mel, lf0, pos)
        utt_feat,utt_mel, utt_lf0 = self.sample_frame(utt_feat, utt_mel, utt_lf0, pos)
        # print(feat.shape, mel.shape, lf0.shape )
        # print (utt_feat.shape, utt_mel.shape, utt_lf0.shape)
        # exit()

        # return {'feat': torch.from_numpy(np.vstack((feat, utt_feat))),
        #         'mel': torch.from_numpy(np.vstack((mel, utt_mel))),
        #         'lf0': torch.from_numpy(np.vstack((lf0, utt_lf0)))
        # }
        # print(np.vstack((feat, utt_feat)).shape)  
